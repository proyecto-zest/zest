 ...
 * effect, so a stale in-flight request never overwrites a newer one. `retry`
 * re-runs the same request after an error.
 */
export function useRecipeFeed(page: number, filters: RecipeSearchFiltersValue) {
  ...
  useEffect(() => {
    ...
    listRecipes(
      ...
      .then((data) => setResult({ status: 'ok', page, filters, data }))
      .catch((error: unknown) => {
